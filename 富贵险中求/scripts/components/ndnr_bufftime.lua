...
function BuffTime:SyncData(name, buffname, time, visible)
    if self.inst.replica.ndnr_bufftime then
        self.inst.replica.ndnr_bufftime:SyncData(name ..";"..buffname..";"..tostring(time)..";"..tostring(visible))
    end
end

function BuffTime:SyncAllData()
    if self.inst.replica.ndnr_bufftime then
        self.inst.replica.ndnr_bufftime:SyncAllData(GetAllData(self))
    end
end
