local ObsidianTool = Class(function(self, inst)
    self.inst = inst
    self.charge = 0
    self.maxcharge = TUNING.NDNR_OBSIDIAN_TOOL_MAXCHARGES
    self.cooldowntime = TUNING.TOTAL_DAY_TIME / TUNING.NDNR_OBSIDIAN_TOOL_MAXCHARGES
    self.cooltimer = 0
...
function ObsidianTool:SetCharge(num)
    ...
    self.inst:PushEvent("obsidian_charge_delta", {
        old = old,
        new = self.charge,
        max = self.maxcharge
    })
...
function ObsidianTool:OnChargeDelta(old, new)
    ...
    if percentage >= self.red_threshold then
    	...
    	if self.owner then
    		self.owner.AnimState:OverrideSymbol("swap_object", "swap_"..self.tool_type.."_obsidian", "swap_"..self.tool_type.."_red")
    	end
    elseif percentage >= self.orange_threshold then
    	...
    	if self.owner then
    		self.owner.AnimState:OverrideSymbol("swap_object", "swap_"..self.tool_type.."_obsidian", "swap_"..self.tool_type.."_orange")
    	end
    elseif percentage >= self.yellow_threshold then
    	...
    	if self.owner then
    		self.owner.AnimState:OverrideSymbol("swap_object", "swap_"..self.tool_type.."_obsidian", "swap_"..self.tool_type.."_yellow")
    	end
    else
    	-- self.inst.components.floatable:UpdateAnimations("idle_water", "idle")
    	if self.owner then
    		self.owner.AnimState:OverrideSymbol("swap_object", "swap_"..self.tool_type.."_obsidian", "swap_"..self.tool_type)
    	end
    end
