local assets =
{
    ...
    Asset("ATLAS", "images/ndnr_palmtree.xml"),
    Asset("IMAGE", "images/ndnr_palmtree.tex"),
}
...
local builds =
{
	normal = {
		file="palmtree_build",
		file_bank="treepalm",
		prefab_name="ndnr_palmtree",
        regrowth_product="ndnr_palmtree_sapling",
        regrowth_tuning=TUNING.EVERGREEN_REGROWTH,
        grow_times=TUNING.EVERGREEN_GROW_TIME,
        short_loot = {"log", "ndnr_palmleaf"},
        normal_loot = {"log", "log", "ndnr_palmleaf"},
        tall_loot = {"log", "log", "ndnr_palmleaf", "ndnr_coconut", "ndnr_coconut"},
		drop_pinecones=true,
        leif="ndnr_treeguard",
...
local function OnBurnt(inst, immediate)
    local function changes()
        ...
        if GetBuild(inst).drop_pinecones then
            inst.components.lootdropper:AddChanceLoot("ndnr_coconut", 0.1)
        end
...
local function SetOld(inst)
    ...
    if GetBuild(inst).drop_pinecones then
        inst.components.lootdropper:SetLoot({"ndnr_coconut"})
...
local function grounddetection_update(inst)
	...
	if pt.y <= 0.2 then
	    ...
	    for k,v in pairs(ents) do
	    	...
	    	if v and v.components.combat and v ~= inst and owner == nil then  -- quakes shouldn't break the set dressing
...
local function chop_tree(inst, chopper, chopsleft, numchops)
	...
	if inst.components.growable and inst.components.growable.stage == 3 then
		if math.random() <= TUNING.NDNR_PALMTREE_COCONUT_CHANCE then
...
local function spawn_leif(target)
    ...
    leif:SetTreeGuardScale(target.leifscale)
...
local LEIFTARGET_MUST_TAGS = { "ndnr_palmtrees", "tree" }
local LEIFTARGET_CANT_TAGS = { "ndnr_treeguard", "stump", "burnt" }
...
local function tree(name, build, stage, data)
    local function fn()
        ...
        if build == "twiggy" then
            ...
            inst:AddTag("ndnr_palmtrees")
            inst.MiniMapEntity:SetIcon(build == "sparse" and "evergreen_lumpy.png" or "ndnr_palmtree.tex")
        ...
        if data == "stump" then
            ...
            inst.components.workable:SetWorkLeft(1)
            inst.AnimState:PlayAnimation(inst.anims.stump)
            inst.MiniMapEntity:SetIcon(build == "twiggy" and "twiggy_stump.png" or "ndnr_palmtree.tex")
        else
        ...
        inst.ndnr_grounddetection_update = grounddetection_update
...
return  tree("ndnr_palmtree", "normal", 0),
        tree("ndnr_palmtree_normal", "normal", 2),
        tree("ndnr_palmtree_tall", "normal", 3),
        tree("ndnr_palmtree_short", "normal", 1),

        tree("ndnr_palmtree_burnt", "normal", 0, "burnt"),
        tree("ndnr_palmtree_stump", "normal", 0, "stump")