--[[
	...
	1. 准备背包内贴图：
			种子，果实，烤熟，巨大，巨大腐烂
	2. 制作动画：
			种子动画（ndnr_farm_plant_seeds.zip）
			果实动画（ndnr_soybean.zip）
			作物动画（farm_plant_ndnr_soybean.zip）【最麻烦的一个】
	3. 细节：
			图鉴里不显示种子名
				声明一个 STRINGS.NAMES.KNOWN_NDNR_SOYBEAN_SEEDS 就显示了
			图鉴里不显示贴图
				使用 RegisterInventoryItemAtlas() 方法将贴图注册一下
			巨大作物背在身上不显示贴图
				需要制作替换通道的动画，跟做武器的替换通道方式一样
			种子种下后，土堆不显示
				官方的作物解包后是没有土堆通道的，解决办法可以直接使用富贵里大豆的解包文件
				或者先解包官方的动画，然后将土堆的通道补上，再打包，然后将贴图复制到官方的zip包里再次解包就有土堆的通道了
	4. 相关代码：
			ndnr_farm_plant_def.lua    定义作物的文件
			ndnr_farm_plants.lua       初始化作物生长阶段的文件
			ndnr_veggies.lua           初始化种子，果实，巨大作物等等的文件
]]
...
local function MakePlant(plant_def)
    local assets =
    {
		...
		Asset("SCRIPT", "scripts/prefabs/ndnr_farm_plant_defs.lua"),
