local assets = {
    ...
    Asset("IMAGE", "images/ndnr_obsidianaxe.tex"),
    Asset("ATLAS", "images/ndnr_obsidianaxe.xml"),
    Asset("ATLAS_BUILD", "images/ndnr_obsidianaxe.xml", 256),
}
...
local function UpdateDamage(inst, data)
    ...
    inst.components.weapon:SetDamage((1+damage_mod)*TUNING.AXE_DAMAGE)
end
...
local function fn()
    ...
    inst.components.ndnr_obsidiantool.tool_type = "axe"
    ...
    inst.components.tool:SetAction(ACTIONS.CHOP, TUNING.NDNR_OBSIDIANTOOLFACTOR)
    -------
    inst:AddComponent("finiteuses")
    inst.components.finiteuses:SetMaxUses(TUNING.AXE_USES)
    inst.components.finiteuses:SetUses(TUNING.AXE_USES)
    inst.components.finiteuses:SetOnFinished(onfinished)
    inst.components.finiteuses:SetConsumption(ACTIONS.CHOP, 1 / TUNING.NDNR_OBSIDIANTOOLFACTOR)
    -------

    inst:AddComponent("inspectable")
    inst:AddComponent("inventoryitem")
    inst.components.inventoryitem.atlasname = "images/ndnr_obsidianaxe.xml"
...
return Prefab("ndnr_obsidianaxe", fn, assets)
