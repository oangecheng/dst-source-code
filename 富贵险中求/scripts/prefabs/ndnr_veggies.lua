...
VEGGIES["ndnr_soybean"] = MakeVegStats(COMMON, TUNING.CALORIES_SMALL, TUNING.HEALING_SMALL, TUNING.PERISH_FAST, 0,
                                                TUNING.CALORIES_SMALL, TUNING.HEALING_MED, TUNING.PERISH_MED, 0,
                                                {nil, 0.1, 0.75})

VEGGIES["ndnr_soybean"].data_only = false
VEGGIES["ndnr_soybean"].cookable = false
...
local assets_seeds =
{
    ...
    Asset("ANIM", "anim/ndnr_farm_plant_seeds.zip"),
}
...
local function MakeVeggie(name, has_seeds)
	...
	local cookable = VEGGIES[name].cookable
	...
	if has_seeds then
		...
		table.insert(assets, Asset("IMAGE", "images/"..name.."_seeds.tex"))
		table.insert(assets, Asset("ATLAS", "images/"..name.."_seeds.xml"))
		table.insert(assets, Asset("ATLAS_BUILD", "images/"..name.."_seeds.xml", 256))
	end

    local assets_cooked = {}
    if cookable then
        table.insert(assets_cooked, Asset("ANIM", "anim/"..name..".zip"))
        table.insert(assets_cooked, Asset("IMAGE", "images/"..name.."_cooked.tex"))
        table.insert(assets_cooked, Asset("ATLAS", "images/"..name.."_cooked.xml"))
        table.insert(assets_cooked, Asset("ATLAS_BUILD", "images/"..name.."_cooked.xml", 256))
    end
    ...
    if has_seeds then
        ...
        -- table.insert(assets_oversized, Asset("ANIM", "anim/"..PLANT_DEFS[name].swap_body_build..".zip"))
        -- table.insert(assets_oversized, Asset("ANIM", "anim/"..PLANT_DEFS[name].swap_body_rotten_build..".zip"))

        table.insert(assets_oversized, Asset("IMAGE", "images/"..name.."_oversized.tex"))
        table.insert(assets_oversized, Asset("ATLAS", "images/"..name.."_oversized.xml"))
        table.insert(assets_oversized, Asset("IMAGE", "images/"..name.."_oversized_waxed.tex"))
        table.insert(assets_oversized, Asset("ATLAS", "images/"..name.."_oversized_waxed.xml"))
        table.insert(assets_oversized, Asset("IMAGE", "images/"..name.."_oversized_rotten.tex"))
        table.insert(assets_oversized, Asset("ATLAS", "images/"..name.."_oversized_rotten.xml"))
    end
    ...
    local function fn_seeds()
        ...
        inst.AnimState:SetBank("ndnr_farm_plant_seeds")
        inst.AnimState:SetBuild("ndnr_farm_plant_seeds")
    ...
    local function fn()
        ...
        if cookable then
            inst:AddComponent("cookable")
            inst.components.cookable.product = "images/"..name.."_cooked.xml"
        end
    ...
    local function fn_oversized_waxed()
        ...
        inst:AddComponent("inspectable")
        inst:AddComponent("inventoryitem")
        inst.components.inventoryitem.atlasname = "images/"..name.."_oversized_waxed.xml"
        inst.components.inventoryitem.imagename = name.."_oversized_waxed"
        inst.components.inventoryitem.cangoincontainer = false
        inst.components.inventoryitem:SetSinks(true)
    ...
    local function fn_oversized_rotten()
        ...
        inst:AddComponent("inventoryitem")
        inst.components.inventoryitem.atlasname = "images/"..name.."_oversized_rotten.xml"
        inst.components.inventoryitem.imagename = name.."_oversized_rotten"
        inst.components.inventoryitem.cangoincontainer = false
...
for veggiename,veggiedata in pairs(VEGGIES) do
    if veggiedata.data_only ~= nil and veggiedata.data_only == false then
