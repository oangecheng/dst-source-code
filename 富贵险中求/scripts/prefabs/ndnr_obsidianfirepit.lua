...
local assets = {
    ...
    Asset("ATLAS", "images/ndnr_obsidianfirepit.xml"),
    Asset("IMAGE", "images/ndnr_obsidianfirepit.tex"),
}

local prefabs = {"ndnr_obsidianfirefire", "collapse_small", "ash"}
...
local function updatefuelrate(inst)
    inst.components.fueled.rate = TheWorld.state.israining and 1 + TUNING.FIREPIT_RAIN_RATE *
                                      TheWorld.state.precipitationrate or 1
end
...
local function OnHaunt(inst, haunter)
    if math.random() <= TUNING.HAUNT_CHANCE_RARE and inst.components.fueled ~= nil and
...
local function fn()
    ...
    inst.MiniMapEntity:SetIcon("ndnr_obsidianfirepit.tex")
    ...
    inst.components.burnable:AddBurnFX("ndnr_obsidianfirefire", Vector3(0, 0.5, 0))
...
return Prefab("ndnr_obsidianfirepit", fn, assets, prefabs),
    MakePlacer("ndnr_obsidianfirepit_placer", "firepit", "firepit", "preview")
