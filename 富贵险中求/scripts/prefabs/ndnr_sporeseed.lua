...
local data =
{
    small =
    { --Green
        imagename = "spore_small",
        deployname = "green_mushroom",
        build = "mushroom_spore",
        lightcolour = {146/255, 225/255, 146/255},
    },
    medium =
    { --Red
        imagename = "spore_medium",
        deployname = "red_mushroom",
        build = "mushroom_spore_red",
        lightcolour = {197/255, 126/255, 126/255},
    },
    tall =
    { --Blue
        imagename = "spore_tall",
        deployname = "blue_mushroom",
...
local function depleted(inst)
    if inst:IsInLimbo() then
        ...
        inst:RemoveTag("ndnr_sporeseed") -- so crowding no longer detects it
        inst.persists = false
...
local SPORESEED_TAGS = {"ndnr_sporeseed"}
local function checkforcrowding(inst)
    local x, y, z = inst.Transform:GetWorldPosition()
    local spores = TheSim:FindEntities(x,y,z, TUNING.MUSHSPORE_MAX_DENSITY_RAD, SPORESEED_TAGS)
...
local function makespore(data)
    ...
    local function OnDeploy(inst, pt, deployer)
        local mushroom = SpawnPrefab(data.deployname)
        if mushroom then
            mushroom.Transform:SetPosition(pt:Get())
	...
	local function fn()
        ...
        -- inst.components.inventoryitem.atlasname = "images/godtoken.xml"
...
return Prefab("ndnr_blue_sporeseed", makespore(data.tall), assets),
    Prefab("ndnr_red_sporeseed", makespore(data.medium), assets),
    Prefab("ndnr_green_sporeseed", makespore(data.small), assets),
	MakePlacer("ndnr_blue_sporeseed_placer", "mushrooms", "mushrooms", "blue"),
	MakePlacer("ndnr_red_sporeseed_placer", "mushrooms", "mushrooms", "red"),
	MakePlacer("ndnr_green_sporeseed_placer", "mushrooms", "mushrooms", "green")
