...
local actionhandlers =
{
    ...
    --下面这俩都是hack，实际上不会执行，用来把尘蛾调整到特定的sg状态
        ...
        onenter = function(inst, has_just_sneezed)
            ...
            if not has_just_sneezed and math.random() < SNEEZE_CHANCE then
                ...
                if ba ~= nil and ba.target ~= nil and ba.target:IsValid() and ba.target:HasTag("dustable") then
                    ...
                    ba.target:DoTaskInTime(TUNING_MEDAL.MEDAL_DUSTMOTH.DUSTABLE_RESET_TIME + math.random() * TUNING_MEDAL.MEDAL_DUSTMOTH.DUSTABLE_RESET_TIME_VARIANCE, reset_dustable_fn)
...
return StateGraph("medal_dustmoth", states, events, "idle", actionhandlers)
