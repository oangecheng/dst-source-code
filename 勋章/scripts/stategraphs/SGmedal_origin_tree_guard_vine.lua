...
local function teleportahead(inst,pos)
    ...
    local newpos = pos
    if dist > TUNING_MEDAL.MEDAL_ORIGIN_TREE_GUARD_VINE_MOVEDIST * TUNING_MEDAL.MEDAL_ORIGIN_TREE_GUARD_VINE_MOVEDIST then
...
local states=
{
    ...
    State{
        ...
        onenter = function(inst)
            ...
            inst.components.timer:StartTimer("attack_cooldown",TUNING_MEDAL.MEDAL_ORIGIN_TREE_GUARD_VINE_ATTACK_PERIOD)
        end,
        ...
        timeline=
        {
            ...
            TimeEvent(17*FRAMES, function(inst)
                local x,y,z = inst.Transform:GetWorldPosition()
                local targets = TheSim:FindEntities(x, y, z, TUNING_MEDAL.MEDAL_ORIGIN_TREE_GUARD_VINE_ATTACK_RANGE, MUST_TAGS, CANT_TAGS )
...
return StateGraph("SGmedal_origin_tree_guard_vine", states, events, "nub_idle")
