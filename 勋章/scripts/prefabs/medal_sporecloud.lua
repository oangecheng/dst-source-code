...
local prefabs =
{
    "medal_sporecloud_overlay",
}

local AURA_EXCLUDE_TAGS = { "chaos_creature", "playerghost", "ghost", "shadow", "shadowminion", "noauradamage", "INLIMBO", "notarget", "noattack", "flight", "invisible" }
...
local function SpawnOverlayFX(inst, i, set, isnew)
    ...
    local fx = SpawnPrefab("medal_sporecloud_overlay")
...
--加速本源植物生长
local SPOIL_CANT_TAGS = { "INLIMBO", "FX" }
local SPOIL_ONEOF_TAGS = { "origin_pollinationable" }
local function DoAreaGrowing(inst)
    local x, y, z = inst.Transform:GetWorldPosition()
    local ents = TheSim:FindEntities(x, y, z, inst.components.aura.radius, nil, SPOIL_CANT_TAGS, SPOIL_ONEOF_TAGS)
    for i, v in ipairs(ents) do
        if v.Declining ~= nil then
            v:Declining()
        end
    end
end
...
local function fn()
    ...
    inst.components.combat:SetDefaultDamage(TUNING_MEDAL.MEDAL_SPORECLOUD_DAMAGE)--单次伤害

    inst:AddComponent("medal_chaosdamage")--混沌伤害
	inst.components.medal_chaosdamage:SetBaseDamage(TUNING_MEDAL.MEDAL_SPORECLOUD_CHAOS_DAMAGE)

    inst:AddComponent("aura")
    inst.components.aura.radius = TUNING_MEDAL.MEDAL_SPORECLOUD_RADIUS--伤害范围
    inst.components.aura.tickperiod = TUNING_MEDAL.MEDAL_SPORECLOUD_TICK--伤害频率
    inst.components.aura.auraexcludetags = AURA_EXCLUDE_TAGS
    inst.components.aura.pretickfn = DoAreaGrowing--催长
    ...
    inst.components.timer:StartTimer("disperse", TUNING_MEDAL.MEDAL_SPORECLOUD_LIFETIME)--消散
...
return Prefab("medal_sporecloud", fn, assets, prefabs),
    Prefab("medal_sporecloud_overlay", overlayfn, assets)
