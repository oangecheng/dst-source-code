local assets =
{
    ...
    Asset("ANIM", "anim/medal_sinkhole.zip"),
...
local NUM_CRACKING_STAGES = 3--塌陷层数
local COLLAPSE_STAGE_DURATION = 0.1--塌陷速度
--更新贴图
...
local function OnTimerDone(inst, data)
    if data ~= nil and data.name == "nextrepair" then
        ...
        if inst.remainingrepairs <= 0 then
            ...
            inst.components.timer:StartTimer("nextrepair", 0.1)
        end
...
local function start_repairs(inst, repairdata)
    ...
    if inst.teleportpos then
        --踩上去也是要掉下去的嘛
        inst.droptask = inst:DoPeriodicTask(0.6, function()
            local x, y, z = inst.Transform:GetWorldPosition()
            for i, v in ipairs(AllPlayers) do
                if not v:HasTag("playerghost") and v:GetDistanceSqToPoint(x, y, z) < TUNING.ANTLION_SINKHOLE.UNEVENGROUND_RADIUS*TUNING.ANTLION_SINKHOLE.UNEVENGROUND_RADIUS then
                    if not (v.sg:HasStateTag("fallhole") or v.sg:HasStateTag("dead")) then
                        v.sg:GoToState("fall_into_black_hole",{pos=inst.teleportpos,worldid=inst.worldid})--传送了
                    end
                end
            end
        end, 0.3)
    ...
    inst.components.timer:StartTimer("nextrepair", 2)
end
--进一步塌陷
local function donextcollapse(inst)
    inst.collapsestage = inst.collapsestage + 1--塌陷程度+1
    --是否是最后一个阶段
    ...
    if isfinalstage then
        ...
        start_repairs(inst)--最后一个阶段了就开始进入修复阶段了
    else
    ...
    for i, v in ipairs(ents) do
        if v:IsValid() then
            ...
            elseif v.components.combat ~= nil and v.components.health ~= nil  and not v.components.health:IsDead() then
                -- print(inst.teleportpos)
                if isfinalstage then
                    if v:HasTag("player") and inst.teleportpos then
                        if not (v.sg:HasStateTag("fallhole") or v.sg:HasStateTag("dead")) then
                            v.sg:GoToState("fall_into_black_hole",{pos=inst.teleportpos,worldid=inst.worldid})--传送了
                        end
                        --时之伤
                        v.components.health:DoDeltaMedalDelayDamage(TUNING_MEDAL.MEDAL_SINKHOLE_DELAY_DAMAGE)
                    elseif v.components.locomotor == nil then
...
--开始塌陷
local function onstartcollapse(inst,data)
    if data then
        inst.teleportpos=data.pos--传送坐标
        inst.worldid=data.worldid--世界ID
    end
    inst.collapsestage = 0--塌陷程度归零

    inst:AddTag("scarytoprey")

    inst.collapsetask = inst:DoPeriodicTask(COLLAPSE_STAGE_DURATION, donextcollapse)
    donextcollapse(inst)--进一步塌陷
end

-------------------------------------------------------------------------------

local function OnSave(inst, data)
    if inst.teleportpos ~= nil then
        data.teleportpos = inst.teleportpos
    end
    if inst.worldid ~= nil then
        data.worldid = inst.worldid
    end
...
local function OnLoad(inst, data)
    if data ~= nil then
        if data.teleportpos ~= nil then
            inst.teleportpos = data.teleportpos
        end
        if data.worldid ~= nil then
            inst.worldid = data.worldid
        end
...
local function fn()
    ...
    inst.AnimState:SetBuild("medal_sinkhole")
...
return Prefab("medal_sinkhole", fn, assets, prefabs)
