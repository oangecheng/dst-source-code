...
local SHARE_TARGET_DIST = 30--仇恨共享距离
...
local function onworked(inst, worker, workleft)
    if workleft and workleft>0 then
        if math.random() < (TUNING_MEDAL.MEDAL_ORIGIN_MOSQUITO_WORKLEFT-workleft)/TUNING_MEDAL.MEDAL_ORIGIN_MOSQUITO_WORKLEFT then
...
local function mosquito()
    ...
    inst:SetStateGraph("SGmedal_origin_mosquito")
    ...
    inst.components.workable:SetWorkLeft(TUNING_MEDAL.MEDAL_ORIGIN_MOSQUITO_WORKLEFT)
    ...
    inst.components.health:SetMaxHealth(TUNING_MEDAL.MEDAL_ORIGIN_MOSQUITO_HEALTH)

    ------------------
    inst:AddComponent("combat")
    inst.components.combat.hiteffectsymbol = "body"
    inst.components.combat:SetDefaultDamage(TUNING_MEDAL.MEDAL_ORIGIN_MOSQUITO_DAMAGE)
    inst.components.combat:SetRange(TUNING_MEDAL.MEDAL_ORIGIN_MOSQUITO_ATTACK_RANGE)
    inst.components.combat:SetAttackPeriod(TUNING_MEDAL.MEDAL_ORIGIN_MOSQUITO_ATTACK_PERIOD)
    ...
    inst.components.medal_chaosdamage:SetBaseDamage(TUNING_MEDAL.MEDAL_ORIGIN_MOSQUITO_CHAOS_DAMAGE)

    MakeHauntablePanic(inst)
    AddHauntableCustomReaction(inst, function(inst, haunter)--作祟有概论直接爆炸
...
return Prefab("medal_origin_mosquito", mosquito, assets, prefabs)
