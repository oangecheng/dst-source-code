...
function BuffTime:UpdateUI()
    if self.owner.replica.legend_bufftime == nil then return end
    local hudscale = TheFrontEnd:GetHUDScale()
    local i = 0
    local items = self.owner.replica.legend_bufftime:GetItems()
    for k, vv in pairs(items) do
        local v = TUNING.LEGEND_BUFFTIMES[vv.buffname]
        if v then
